...
test('ADC value lower than the minimal value of the table', () => {
  const index = LibTable.nextValueIndex(TBL_BOSCH_TEMP, 0.037)
  expect(index).toBe(1)
})
...
test('ADC value equal to the minimal value of the table', () => {
  const index = LibTable.nextValueIndex(TBL_BOSCH_TEMP, 0.038)
  expect(index).toBe(1)
})
...
test('ADC value equal to the maximal value of the table', () => {
  const index = LibTable.nextValueIndex(TBL_BOSCH_TEMP, 0.961)
...
test('ADC value greather than the maximal value of the table', () => {
  const index = LibTable.nextValueIndex(TBL_BOSCH_TEMP, 0.962)
  expect(index).toBe(18)
})
...
test('Typical ADC value between two values', () => {
  const index = LibTable.nextValueIndex(TBL_BOSCH_TEMP, 0.450)
...
test('Typical ADC value equal to a value', () => {
  const index = LibTable.nextValueIndex(TBL_BOSCH_TEMP, 0.152)
  expect(index).toBe(7)
})
